	...
	<div id="aMd_wrap">
		...
		<input type="hidden" name="page" value="${page}" />
			<table id="aMd_t">
				<tr>
					...
					<td><input name="mem_id" size="14" value="${m.mem_id}"	readonly /></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><input name="mem_zip" id="mem_zip" size="5"
						...
						id="mem_zip2" size="5" value="${m.mem_zip2}" readonly /> <input
						type="button" value="우편검색" onclick="post_check();" /></td>
				</tr>
				<tr>
         ...
         ${'selected'}</c:if>>${p}</option>
						...
						value="${m.mem_phone02}" />- <input name="mem_phone03"
						id="mem_phone03" size="4" maxlength="4" value="${m.mem_phone03}" />
					</td>
				</tr>
				<tr>
					...
					<td><input name="mail_id" id="mail_id" size="14"
						...
						id="mail_domain" size="12" value="${m.mail_domain}" readonly /> <select
        ...
        ${'selected'}</c:if>>${mail}</option>
				...
				<tr>
					...
					<td><input name="mem_state" size="2" value="${m.mem_state}" /></td>
				</tr>
				<tr>
					<th class="left_box">탈퇴사유</th>
					<td><textarea name="mem_delcont" rows="8" cols="33">${m.mem_delcont}</textarea></td>
				</tr>
				<tr>
					<th colspan="2"><input type="submit" value="수정" /> <input
						type="reset" value="취소" onclick="$('#mem_pwd').focus();" /> <input
						type="button" value="회원목록"
						onclick="location='admin_member_list?page=${page}';" /></th>
				</tr>
			</table>
		</form>
	</div>
