...
<body>
   ...
   &nbsp;<%--답변글 들여쓰기 --%>
									...
									<img src="./resources/images/bbs/AnswerLine.gif" />
									<%--답변글 이미지 출력부분 --%>
...
get방식으로 &구분하면서 전달된다. --%></td>
    ...
    [다음]
			...
			<div id="bList_menu">
				<input type="button" value="글쓰기" onclick="location='board_write?page=${page}';" />
				<c:if test="${(!empty find_field) && (!empty find_name)}">
                <input type="button" value="전체목록" onclick="location='board_list?page=${page}';" />
                </c:if>
			</div>
