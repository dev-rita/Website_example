...
<div id="bList_wrap">
 ...
 <div class="bList_count">글개수: <strong>${totalCount}</strong></div>
 <table id="bList_t">
  <tr>
  ...
  <c:if test="${!empty blist}">
   ...
   <tr>
    <td align="center">
    <c:if test="${b.bbs_step ==0}"><%--원본글일때만 글 그룹번호를 출력 --%>
    ${b.bbs_ref}
    </c:if>
    </td>
    <td>
    <c:if test="${b.bbs_step != 0}"><%--답변글일때만 실행됨 --%>
     <c:forEach begin="1" end="${b.bbs_step}" step="1">
     &nbsp;<%--한칸의 빈공백처리 특수문자, 답변글 들여쓰기 --%>
     </c:forEach>
     <img src="./resources/images/bbs/AnswerLine.gif" /><%--답변글 이미지 --%>
    </c:if>
     <a href="bbs_cont?bbs_no=${b.bbs_no}&page=${page}&state=cont">${b.bbs_title}</a><%-- 주소창에 노출되는 get방식으로 bbs_cont?bbs_no=
     번호&page=쪽번호&state=cont 구분값이 각각 3개의 피라미터이름에 값을 담아서 전달 --%>
    </td>
 ...
 <%--페이징  --%>
  <div id="bList_paging">
   <%--검색 전 페이징 --%>
   <c:if test="${(empty find_field) && (empty find_name)}"><%-- 검색필드와 검색어가 비어 있다면 --%>
    <c:if test="${page <= 1}">
     [이전]&nbsp;
    </c:if>
    <c:if test="${page>1}">
   ...
   <%--검색후 페이징 --%>
   <c:if test="${(!empty find_field) && (!empty find_name)}"><%-- 검색필드와 검색어가 있다면 --%>
    <c:if test="${page <= 1}">
     [이전]&nbsp;
    </c:if>
    <c:if test="${page>1}">
  ...
  <div id="bFind_wrap">
   ...
   <input name="find_name" size="14" value="${find_name}" /><%--type을 생략하면 기본값이 text이다. --%>
