...
<script>
	function w_check() {
		...
		return false;
		 ...
		 try {
				var result= confirm("글 수정을 하시겠습니까?");
				if(result==true){//확인
					alert("글 수정이 완료되었습니다!");
...
<div class="line" id="article">
	...
	<form method="post" action="board8_ok.jsp" onsubmit="return w_check();"><!-- onsubmit="return w_check();" -->
		...
		<input class="btn_reset" id=reset type="reset" value="취소" /> <input class="btn_submit" id="submit" type="submit" value='저장' /> <br/><br/>
	...
	<script>
		$(document).ready(function(){
			$("#reset").click(function(){
				if(confirm("글 수정을 취소 하시겠습니까?")==true){//확인
				alert('글 수정을 취소합니다.');
				history.go(-1);
				}else{ return false;}
					//this.reset(); 쓴거 없앰
